...
interface KpiCardProps {
  ...
  valor: string | number;
  unidad?: string;
  cambio: number;        // positivo = sube, negativo = baja
  descripcion: string;
  color: string;         // color de acento
  data: { v: number }[]; // mini sparkline
  icono: React.ReactNode;
  destacado?: boolean;   // primera card más grande / coloreada
}
...
const kpis: Omit<KpiCardProps, 'color'>[] = [
  {
    titulo: 'Ventas Nacionales',
    valor: '$148.3M',
    cambio: 8.4,
    descripcion: 'Incremento vs mes anterior',
    data: spark(148, 8, 0.12),
  ...
  },
  {
    ...
    valor: '92.6%',
    cambio: 2.1,
    descripcion: 'Cobertura promedio nacional',
    data: spark(92, 8, 0.05),
    icono: <Package size={22} />,
  },
  {
    titulo: 'Riesgo de Desabasto',
    valor: '7.4%',
    cambio: -3.2,
    descripcion: 'SKUs en nivel crítico',
    data: spark(7, 8, 0.2),
...
const KpiCard: React.FC<KpiCardProps> = ({
  ...
  const cambioColor = neutro
    ? colores.textoMedio
    : positivo
    ? colores.exito
    : colores.peligro;

  const cardBg = destacado
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div
          style={{
            ...
            height: '32px',
            borderRadius: '50%',
            background: destacado ? 'rgba(255,255,255,0.2)' : `${color}18`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            color: iconoColor,
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            <Line
              type="monotone"
              dataKey="v"
              stroke={destacado ? 'rgba(255,255,255,0.6)' : color}
              strokeWidth={2}
              dot={false}
            />
            <Tooltip
      ...
      >
        <div
          style={{
            ...
            alignItems: 'center',
            gap: '3px',
            color: destacado ? 'rgba(255,255,255,0.9)' : cambioColor,
            fontSize: '12px',
            fontWeight: '700',
          }}
        >
        ...
        <span style={{ fontSize: '11px', color: textoSecundario }}>
          {descripcion}
        </span>
      </div>
    </div>
...
export const DashboardEjecutivo: React.FC = () => {
  ...
  // Simula "actualización en vivo" cada 8s
  useEffect(() => {
  ...
  const ahoraStr = new Date().toLocaleTimeString('es-MX', {
    hour: '2-digit',
    minute: '2-digit',
  });
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div
              style={{
                ...
                animation: 'pulseEjec 2s infinite',
              }}
            />
            <h3
              style={{
        ...
        <div
          style={{
            ...
            padding: '6px 14px',
            borderRadius: '20px',
            background: `${colores.primario}12`,
      ...
      >
        {[
          ...
          >
            ...
            <div>
              <div style={{ fontSize: '15px', fontWeight: '700', color: colores.textoClaro }}>
                {m.valor}
              </div>
