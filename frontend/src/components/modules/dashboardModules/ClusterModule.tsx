...
export const ClustersSegmentacion: React.FC<ClustersSegmentacionProps> = ({ onNavigate }) => {
  ...
  const clusterDetalle = clusterSeleccionado
    ? clusters.find(c => c.id === clusterSeleccionado) ?? null
    : null;
    ...
    <div style={{ backgroundColor: colores.fondoSecundario, borderRadius: '24px', border: `1px solid ${colores.borde}`, padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <h3 style={{ fontSize: '15px', fontWeight: 'bold', color: colores.textoClaro, margin: 0 }}>Clusters de Segmentación IA</h3>
      ...
      <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
        {filtros.map(f => (
          <button key={f} onClick={() => setFiltroActivo(f)}
            style={{ padding: '4px 9px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '9px', fontWeight: '600', transition: 'all 0.2s',
      ...
      <div style={{ backgroundColor: colores.fondoTerciario, borderRadius: '14px', padding: '12px', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <p style={{ fontSize: '10px', color: colores.textoMedio, margin: 0 }}>Intensidad demanda vs Riesgo de desabasto · tamaño = volumen casos</p>
        </div>
      ...
      {/* Lista clusters ordenados por riesgo */}
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {clusters.map(c => (
            <button key={c.id} onClick={() => setClusterSeleccionado(prev => prev === c.id ? null : c.id)}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 11px', backgroundColor: clusterSeleccionado === c.id ? `${c.color}15` : colores.fondoTerciario, borderRadius: '10px', border: `1px solid ${clusterSeleccionado === c.id ? c.color + '44' : 'transparent'}`, cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left', width: '100%' }}
            >
              <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: c.color, flexShrink: 0, boxShadow: `0 0 6px ${c.color}88` }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: colores.textoClaro, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.nombre}</p>
                <p style={{ fontSize: '9px', color: colores.textoMedio, margin: '1px 0 0 0' }}>{c.casos.toLocaleString()} casos · {c.skus} SKUs</p>
              </div>
